...
class DummyFeedback:
  ...
  def __init__(self, outputs, source_one_hot, adjacency):
    self.outputs = outputs
    self.features = object()
    self._source_one_hot = source_one_hot
    self._adjacency = adjacency

  def __getitem__(self, idx):
    if idx != 0:
      raise IndexError(idx)
    return [[None, DummyDataPoint(self._source_one_hot), DummyDataPoint(self._adjacency)]]


class BellmanFordPluginTest(unittest.TestCase):
  ...
  def _load_plugin_with_stubs(self):
    ...
    clrs_pkg.evaluate = lambda outputs, preds: {"score": np.asarray(0.77)}
    ...
    reports_module.build_bf_result_dict = lambda **kwargs: {"base": True}
    ...
    bf_sampling_module = types.ModuleType(
        "clrs._src.multi_sol.sampling.bellman_ford")
    bf_sampling_module.sample_beamsearch = lambda adjacency, source, data: []
    bf_sampling_module.sample_greedysearch = lambda adjacency, source, data: []
    self._install_module("clrs._src.multi_sol.sampling.bellman_ford",
                         bf_sampling_module)

    bf_validation_module = types.ModuleType(
        "clrs._src.multi_sol.validation.bellman_ford")
    bf_validation_module.check_valid_bf_paths = (
        lambda adjacency, parent_tree, source: True)
    self._install_module("clrs._src.multi_sol.validation.bellman_ford",
                         bf_validation_module)

    uniqueness_module = types.ModuleType("clrs._src.bf_uniqueness_check")
    uniqueness_module.check_uniqueness_bf = lambda **kwargs: (0.1, 0.2, 0.3)
    self._install_module("clrs._src.bf_uniqueness_check", uniqueness_module)

    plugin_path = (
      self._repo_root / "clrs" / "_src" / "multi_sol" / "algorithms" /
      "bellman_ford" / "plugin.py")
    spec = importlib.util.spec_from_file_location(
        "clrs._src.multi_sol.algorithms.bellman_ford.plugin", str(plugin_path))
  ...
  def test_distribution_validation_runs_when_enabled(self):
    ...
    out = plugin.evaluate_bf_multisol_batch(
