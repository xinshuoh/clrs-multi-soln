...
def extract_beam(outs_or_preds, batch):
  pi_trees = []
  for (
      adjacency,
      source,
      prob_matrix,
  ) in extractor_utils.iter_adjacency_source_prob_matrices(batch.adjacency, batch.source_nodes,
                                                           outs_or_preds):
...
def BF_greedysearch(adjacency, source, prob_matrix, beamwidth=3):
  ...
  for node in range(len(prob_matrix)):
    ...
    best_parent = extractor_utils.highest_probability_real_neighbour(adjacency, prob_matrix, node)
    for _ in range(10):
      candidates = [
